...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!categoryData.success)
      return NextResponse.json({ status: false, message: "Invalid Category" });
    ...
    if (categoryData.data.action === "update") {
      await Categories.updateOne(
        { _id: new Types.ObjectId(categoryData.data.id) },
        { $set: { category: categoryData.data.category } },
      );
    ...
    if (categoryData.data.action === "delete") {
      const hasProduct = await Products.findOne({
        categoryId: new Types.ObjectId(categoryData.data.id),
      });
      if (hasProduct)
        return NextResponse.json({
          status: false,
