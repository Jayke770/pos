...
export function Navigation() {
	return (
		<header
			className={
				"fixed z-20 md:static md:p-4 bottom-0 w-full md:w-auto md:h-full md:left-0 md:top-0 transition-all"
			}
		>
				...
				<div className=" p-2 flex flex-row md:flex-col gap-2">
					{menus?.map((menu) => (
						<motion.a
							...
							whileTap={{ scale: 0.9 }}
							className={cn(
								"w-full p-2 md:p-3 flex rounded-xl flex-col items-center gap-1 hover:bg-accent transition-colors cursor-pointer appearance-none",
							)}
						>
